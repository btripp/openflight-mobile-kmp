...
- R5a ✅ merged. New `core:insights` (KMP, depends only on `core:model`): `UnitSystem` (IMPERIAL/METRIC) + `convertSpeedFromMph`/`convertDistanceFromYards`/`formatSpeed`/`formatDistance` (space before unit, reuses `ShotMetricFormatter.number`)/`formatDegrees` (tight, no space); `ClubStats`/`computeClubStats` (web `computeStats` port, nulls ignored for club speed/smash only) + `ClubChip`/`computeClubChips` (first-appearance order); `buildShotsCsv`/`buildShotsCsvFilename` (web `ShotList.tsx` export minus comparator columns, minus fields `ShotEvent` doesn't carry; numbered oldest-first). `core:data`: `SettingsRepository.units`/`setUnits` (DataStore key `unitSystem`, default IMPERIAL) with **default interface bodies** so out-of-scope fakes (`feature:calibration`, `feature:range`, `composeApp/PreviewShotRepository`) stay source-compatible; `ShotRepository.deleteShot`/`clearHistory` (local-only, default no-op bodies, same compatibility reason) and `shutdownPi()` (throws `PiShutdownUnsupportedException` unless the active transport is Wi-Fi). `core:network`: `PiControlClient.shutdown(host)` for `POST /api/shutdown`, MockEngine-tested. `feature:dashboard` (commonMain/commonTest only): `DashboardUiState` gained `units`/`clubStats`/`clubChips` (all with defaults — **no compile fix needed in `DashboardScreen.kt`**, R1's Compose migration is unaffected) and a `DashboardEffect.NewShot` one-shot `Channel`, fired only when `history`'s newest `eventId` changes after the VM's first collected value (never for the replay a fresh collector sees, never for a transport replay, since the repository's dedup means `history` doesn't re-emit for those). New `feature:session` (commonMain VM only, `openflight.kmp.compose` plugin like its siblings but no `@Composable` code): `SessionViewModel`/`SessionUiState` (club tabs + stats for the selected tab) /`SessionEvent` (`SelectClub`/`DeleteShot`/`ClearHistory`/`ExportCsv`) /`SessionEffect.CsvReady` (CSV text + filename, full history exported oldest-first regardless of the selected tab, matching the web UI). Registered in `composeApp/AppKoin.kt`'s `appModules` (one line) + `api(projects.feature.session)`. Verification (`spotlessCheck detekt allTests :androidApp:assembleDebug :composeApp:linkDebugFrameworkIosSimulatorArm64`) green on both Android host tests and `iosSimulatorArm64Test`; two pre-existing warnings untouched (`DefaultShotRepository.kt` unused-expression from S6, `CalibrationScreen.kt` from the concurrent R1 agent).
- R6a ✅ `core:socketio` (Engine.IO v4/Socket.IO v5 text codec incl. `\x1e` payloads, `SocketIoClient` with 1→15 s backoff, `pingInterval+pingTimeout` watchdog, events accepted before the `40` ack since Flask-SocketIO's connect handler broadcasts first; `KtorWebSocketTransport` = **WebSocket-only, no polling handshake**: python-engineio 4.14 + simple-websocket 1.1 in `threading` mode accepts `transport=websocket` directly, verified live; `MjpegParser`). Ktor 3.6's client WebSockets plugin lives in ktor-client-core (`ktor-client-websockets` is an empty shell), so no catalog entry was added. `core:model` `pi.*` models (`ShotDetail` = full `shot_to_dict` + swing rows, lenient) and `core:model` now `api(kotlinx-serialization-json)`. `core:data` `PiSessionRepository` (Koin single; Wi-Fi only, `WifiOnlyFeatureException(BLUETOOTH|NOT_CONNECTED)`, `notices` for server errors). **Enrichment key = `timestamp`** (SSE `event_id` is a fresh UUID per publish; timestamps verified identical live) → `shotDetails`/`detailFor(ShotEvent)`. Live `--mock` :8092 run (`ManualPiSessionE2ETest`, @Ignore): connect, session_state, simulate×2 + SSE enrichment found, delete + "Shot not found", set_player, training implement (+ error), radar_config (set → "Radar not connected"), upload_cloud → error "Cloud uploader is not linked.", debug on/off, camera `toggle_camera` → unavailable and `GET /camera/stream` → 503 "Camera not available", clear_session, shutdown → `shutdown_ack` then link Reconnecting. Not integrated into `ShotRepository`/app start yet (R6b/R6 integration).
- ⚠ R6b integration note: SSE/BLE `event_id` is a fresh random UUID per publish (build_shot_event); the stable cross-channel key is `timestamp`. Server delete_shot is keyed by timestamp; R5a's local ShotRepository.deleteShot is keyed by eventId → R6b must route deletes by timestamp when Socket.IO is connected. Socket.IO runs WebSocket-only on the mock (python-engineio 4.14 + simple-websocket).
- R1 ✅ CMP removed (no `org.jetbrains.compose` in any .kts/.toml). Jetpack: Compose BOM 2026.09.00 (Compose 1.12.1, material3 1.4.0), lifecycle 2.11.0 (KMP `lifecycle-viewmodel` in commonMain), navigation-compose 2.10.2, koin-androidx-compose/koin-core-viewmodel 4.2.2 (BOM). Layout: Android-only `feature:<name>:ui` modules (plugin `openflight.android.library.compose`, same Kotlin packages as their KMP module) instead of androidMain, so the Compose compiler never touches iOS; `core:designsystem` is the same plugin (`Of*` API unchanged, OfMetricTest → src/test). Feature commonMain = VM/UiState/events/Koin module/test tags/pure math only; `verifyNoComposeInCommonMain` (KMP plugin, runs in allTests/check) + CI grep enforce it. Range: `RangeProjection`/`FlightGeometry`/`playbackSeconds` stay common and now return `ScreenPoint` (was `Offset`), made public; ui converts via `toOffset()`. `KeepScreenOn`/`rememberReduceMotionEnabled` are now plain internal Android composables in the ui modules (iOS actuals deleted; SwiftUI does idleTimerDisabled / accessibilityReduceMotion). `composeApp` → `shared` (static `Shared.framework`, exports core:model/data/insights + all feature:*; `initKoin(extraModules, appDeclaration)`, LaunchOptions, PreviewShotRepository; iosMain `startKoinForIos()` + `KoinHelper` VM getters). androidApp owns `OpenFlightApp()` (was `App()`), `AppNavHost`, `TransportPermissionRequest`, intent-extra parsing, `initKoin { androidContext }`; no Session route (R5b). `DrivingRangeFlowTest` → androidApp/androidTest: it `stopKoin()`s the Application's graph and grants ACCESS_LOCAL_NETWORK up front (the app manifest declares it, unlike the old library test APK). iosApp = SwiftUI placeholder showing `AppInfoKt.platformSubtitle()`. Verified: spotlessCheck detekt allTests assembleDebug :shared:linkDebugFrameworkIosSimulatorArm64 green; device tests on Pixel_10_Pro_XL dashboard 12 / calibration 10 / range 8 / app 2 (= before); xcodebuild + sim launch OK; live mock :8093 dashboard/history/calibration 409 match S7/S8c shots, and range replay frames pixel-identical to a pre-R1 (68bb385) APK after landing (in-flight frames differ only at the ball tip, timing). Note: koin-androidx-compose still pulls org.jetbrains.compose.* transitively (Gradle-metadata redirects to androidx on Android).
- R2 ✅ iOS interop + SwiftUI shell + Dashboard. **Interop = KMP-NativeCoroutines 1.0.6** (Gradle plugin on `shared` only, compiler-plugin mode, no KSP; Swift package `KMPNativeCoroutinesAsync` exact 1.0.6). SKIE was ruled out because its docs cap Kotlin at 2.4.10 (0.10.14). Evidence is in the ADR 0001 "Decision (R2)" section. Bridge: `shared/iosMain/NativeViewModels.kt` holds one annotated extension pair per VM (`@NativeCoroutinesState val XViewModel.state` → Swift `state`/`stateFlow`; `@NativeCoroutines val XViewModel.sideEffects`), and `ViewModelStoreHolder` runs `onCleared()` through a `ViewModelStore`. Swift `ViewModelHost<VM: SharedViewModel>` (`iosApp/iosApp/Bridge/ViewModelHost.swift`, an `ObservableObject` held in a `@StateObject`) collects on the MainActor, exposes `send(_:)` and `collect(_:perform:)` for effects, and clears on deinit. To add a VM: an extension pair in NativeViewModels.kt plus `extension XViewModel: SharedViewModel {}`. `KoinHelper.shotRepository()` is new; the `scenePhase` handler calls start() on .active and stop() on .background. `AppRoot` has a `NavigationStack` with `AppRoute.calibration/.range`, each a one-line `PlaceholderScreen` (**R3a/R3b replace only their own `case` line in `AppRoot.swift`**), and `--range-mode` pushes `.range`. The DashboardView port (ContentView layout/copy, gold `Theme` = OfColorTokens) covers the transport picker, host-on-submit, status/Retry, metrics ("—", tight "9.5°"), the club menu gated on `clubMenuEnabled` with an inline dismissable error, history, and a light haptic on `NewShot`. Unit/range chips aren't shown, matching Android. Xcode additions: targets `iosAppTests` (hosted, `-disable-autolink-framework Shared` so it uses the app's single Kotlin runtime) and `iosAppUITests`, plus the shared scheme `iosApp`. Verified: spotlessCheck detekt allTests assembleDebug linkDebugFrameworkIosSimulatorArm64 all green. On the iPhone 17 Pro sim (UUID DE4491D0-…), `xcodebuild test` ran 3 unit tests and 5 UI tests with 0 failures (the live test ran with `TEST_RUNNER_OPENFLIGHT_LIVE_HOST=localhost:8094`). Live mock on :8094: XCUITest taps changed the club from driver to **7-iron** (`POST /api/club`, then `GET /api/club` returned `7-iron`), and 3 fired shots streamed into the dashboard and history (screenshots `r2-ios-*.png` in the scratchpad). Note: the selected club persists across UI-test launches, so the menu-item query excludes `dashboard.clubSelector`.
- R6b ✅ shared VMs + integration (no UI). **Lifecycle:** `DefaultShotRepository(piSession = …)` starts/stops `PiSessionRepository` inside `ShotRepository.start()/stop()` (no platform change; the Pi session itself follows transport+host, so it's Wi-Fi-only, same host). **Routing:** `ShotRepository.deleteShot(eventId)` + new `deleteShotByTimestamp(ts)` + `clearHistory()` always act locally and, only while `linkState == Connected`, also emit `delete_shot{timestamp}` / `clear_session` (failures logged). New `core:model.pi.PiFeatureAvailability` (Available / Unavailable("Requires Wi-Fi"|"Not connected")) gates every Wi-Fi-only action. `core:insights`: `ShotEnrichment` (+`ConfidenceLevel` 3-dot high/med/low/experimental, `SpinSource` radar/estimated, `formatCarryRange`), `SwingSpeedStats`/`computeSwingSpeedStats` (web filter port), `computeDetailStats`/`computeDetailClubChips`/`SessionStats.toClubStats`, `ExportShot` + `buildExportCsv` (web columns player/mode/implement/openflight_speed_mph/reading_count/trigger_speed_mph/duration_ms/peak_magnitude appended only when a `ShotDetail` exists; R5a output unchanged otherwise). Dashboard: `Live.enrichments` (eventId→`ShotEnrichment`, appended with default so `DashboardScreen`/its device tests compile unchanged), `latestEnrichment`, `enrichmentFor(shot)`. Session: `SessionSource` PI (server rows + server stats for "All") / LOCAL, `SessionShotRow`, `swingStats`, `SimulateShot` (mock only), `DeleteShot(id)` routes by id shape (history eventId → `deleteShot`, else `deleteShotByTimestamp`). New KMP modules `feature:training` (implement picker from server `TRAINING_IMPLEMENT_LABELS` grouped like the web UI, legacy `speed-stick-*` aliases hidden), `feature:camera` (`CameraPhase`, toggles, `frames: Flow<ByteArray>` shared MJPEG, `RetryStream`), `feature:settings` (units, player, sims, radar sliders w/ web ranges + mock rules, trigger diagnostics, debug+log path, cloud, shutdown confirm flag, transport/link) and test-only `core:testing` (FakePi/Shot/Settings repositories). `shared` exports + `appModules` + `KoinHelper.training/camera/settingsViewModel()` appended; `KoinHelperIosTest` resolves all 7 VMs from the real iOS graph. Verified: spotlessCheck detekt allTests assembleDebug linkDebugFrameworkIosSimulatorArm64 green; live `--mock --web-port 8095` throwaway run: start→link Connected, 2 simulated SSE shots both enriched by timestamp, `deleteShot(eventId)` removed it on the Pi by timestamp, clear emptied both, stop→Idle, BLE→WifiOnly.
- R3b ✅ SwiftUI Driving Range (dcaffbc). Ported `DrivingRangeView`/`RangeSceneView`/`RangeSceneController` (non-AR `ARView`, same meshes/colours/camera/72|120-segment box tracer as the reference; `RangeQualityProfile.current` = Swift memory check) and `RangeMetricsOverlay` under `iosApp/iosApp/Range/`, rendering the shared `DrivingRangeUiState`. Bridge: `@NativeCoroutinesState val DrivingRangeViewModel.state` (no effects) + `extension DrivingRangeViewModel: SharedViewModel {}` (in DrivingRangeView.swift, to avoid touching ViewModelHost.swift). **`shared` now also exports `core:flight`** (api + export), so Swift reads `FlightTrajectory.point(at:)`, `RangeSceneDescription.companion.standard`, `RangeCameraPlanner`, `RangeTracerStyle.highVisibility` directly — no flat-array helper needed. Axes: core:flight +z downrange → scene −z via `scenePosition` (as reference/Android `flightToScene`). Playback keyed on `ActiveFlight.playbackId` (replay restarts); `activeFlight == nil` suspends; duration from shared `playbackSeconds(trajectory:reduceMotion:)` with `accessibilityReduceMotion` (view-side, as Android — the VM takes no reduce-motion input). Suspends on `scenePhase != .active`, onDisappear and Exit. Range hides the nav bar and Exit `dismiss()`es, so `DashboardUITests.testCalibrateAndRangeOpenTheirScreensAndGoBack`'s range half now uses `range.exit`. `--preview-flight` autoplays via `DrivingRangeView(autoplay: launchOptions.previewFlight)`; LaunchOptions already parsed `--transport wifi --host h:p` on iOS. Verified: spotlessCheck detekt allTests assembleDebug linkDebugFrameworkIosSimulatorArm64 green; iPhone 17e sim (67A62C8E…) `xcodebuild test`: 3 unit + 7 UI passed, live test skipped. Live: mock on **:8098** (8097 held by an unrelated long-running `node server.mjs`), app launched `--range-mode --transport wifi --host localhost:8098`, 3 shots @0.5 s (server carries 279/251/261): 279 flew and landed → 261 flew, 251 never shown; carry matches server; tracer runs downrange from the tee (flat arc = S9 finding). Screenshots `r3b-ios-*.png` in scratchpad.
- R3b ✅ merged (6d0f4b8), orchestrator viewed r3b screenshots: real RealityKit 3D range, newest-wins OK. iOS polish TODO (R7b/R4): (1) degree spacing '11.4 °' in Swift overlay/dashboard — use shared formatDegrees; (2) metrics overlay covers lower half incl. landing area — collapse/compact during Flying; (3) flat arcs (known, matches reference). Port 8097 is held by an unrelated node process.
- R3a ✅ SwiftUI Calibration. Bridge: `shared/iosMain/NativeViewModels.kt` gained `@NativeCoroutinesState val CalibrationViewModel.state` (no effects — `CalibrationViewModel` has none) and `ViewModelHost.swift` gained `extension CalibrationViewModel: SharedViewModel {}`. `iosApp/iosApp/Calibration/CalibrationView.swift` ports `RadarCalibrationView.swift`'s layout/copy over the shared `CalibrationUiState`, split like Android's `CalibrationScreen.kt`/`CalibrationMeasurementCard.kt`: instructions card; the Wi-Fi host field (submits on Go, like the dashboard) or the Bluetooth readiness card gated on `bluetoothReady`; the live/stable mount-tilt and roll cards (green once `isStableAverage`), with roll-out-of-range in warning color; progress + "n / 120 samples" before a measurement exists, then the stability text and readiness message; Apply gated on `applyEnabled`; on Applied, "Saved TI tilt: X°" plus the enclosure-pitch summary (`responseSummary`, matching both Android and the reference); on Failed, the server's message verbatim; "Motion unavailable" (with a description) when the sensor is `Unavailable`. `PhoneOrientationMeasurement.companion`/`SensorsPhoneOrientationDisplayAngles` (note the `Sensors`-prefixed Swift name — the `core.sensors` package collides with another type) supply the thresholds and angle fields; verified the exact Swift/ObjC bridging names (e.g. `SensorUiStateSampling`, `SubmitUiStateApplied`, `CalibrationTestTags.shared.*`) by inspecting the built `Shared.framework`'s generated header before writing the view, rather than guessing. Screen-awake: confirmed R1 deleted the iOS `KeepScreenOn` actual (Android/SwiftUI now diverge, per R1's log), so `CalibrationView` sets `UIApplication.shared.isIdleTimerDisabled` directly, toggled on `sensor is SensorUiStateSampling` and cleared `onDisappear`. `AppRoot.swift`: replaced only the `case .calibration:` line with `CalibrationView()`; the `case .range:` placeholder (R3b) was untouched. New `iosApp/iosAppUITests/CalibrationUITests.swift`: `testMotionUnavailableShowsAndApplyIsDisabled` (dashboard → Calibrate, asserts "Motion unavailable" and a disabled `calibration.apply`) and `testDoneReturnsToTheDashboard` (`calibration.done` → back at `dashboard.calibrateRadar`). Verified: `spotlessCheck detekt allTests :androidApp:assembleDebug :shared:linkDebugFrameworkIosSimulatorArm64` green. On the iPhone 17 Pro Max sim (F9696659-…, chosen to avoid the booted iPhone 17 Pro the user was using), `xcodebuild test` (after a clean build) ran 3 `iosAppTests` + 7 `iosAppUITests` (1 skipped: the live host test) with 0 failures, including the pre-existing R2 dashboard tests. Screenshots `r3a-ios-dashboard.png` and `r3a-ios-calibration.png` in the scratchpad, taken via a temporary XCUITest `XCTAttachment` (added, run once, screenshot extracted with `xcresulttool export attachments`, then the temp test file deleted before committing) since no `idb`/`cliclick`/accessibility-enabled `osascript` is available in this environment (same gap R2/S8c hit). No live server run: per the task, the simulator has no motion so Apply can never actually fire, and port 8094/8096 weren't needed.
- R5b/R6c-Android ✅ Jetpack Compose UI for the parity + full-Wi-Fi VMs. New `feature:{session,training,camera,settings}:ui` (Route owns the VM + effects, stateless Screen, previews, `*TestTags` in the ui module since the shared `commonMain` stays untouched). `core:designsystem`: DM Serif Display (display/headline/titleLarge) + Outfit variable font (everything else, weight via `FontVariation`) bundled as `res/font`, OFL texts in `assets/licenses/`; new wrappers `OfTextButton`, `OfConfirmDialog`, `OfSwitchRow`, `OfSlider` (commits on release), `OfDisabledReason`, `OfChip`, `OfConfidenceDots`, `OfPill`, `OfSwipeToDelete` (M3 1.4 `SwipeToDismissBox(onDismiss)` + a TalkBack "Delete" custom action), `OfBottomBar`, `OfMessageHostState` (snackbars via `OfScaffold(messages, bottomBar)`), `OfMetricPrimary(subtext)`. Dashboard: bottom bar Session/Training/Camera/Settings (`DashboardNavigation`; Range + Calibrate unchanged), units everywhere, 3-dot confidence for Launch/Spin, spin-source subtext (VM label "radar"/"estimated"), carry range or "spin-adjusted" carry, player, club-chips card, `HapticFeedbackType.Confirm` + 600 ms radial gold `ShotFlash` on `DashboardEffect.NewShot`. Camera: `rememberLatestFrame` = `frames.conflate()` + `BitmapFactory` off-main under `repeatOnLifecycle(STARTED)`, only the newest bitmap referenced. CSV: androidApp `FileProvider` (`${applicationId}.fileprovider`, `cache/exports/`) + `ACTION_SEND text/csv` chooser; catalog `androidx-core 1.18.0` (the version activity already resolved). ⚠ koin-compose caches the first Koin per process, so app flow tests that `stopKoin()`/`initKoin` must wrap content in (deprecated) `KoinContext(koin)`; without it `DrivingRangeFlowTest`'s range-mode test failed after any earlier app test (latent before R6c). Verified: spotlessCheck detekt allTests assembleDebug linkDebugFrameworkIosSimulatorArm64 green; device tests on Pixel_10_Pro_XL dashboard 19 (12+7) / session 12 / training 8 / camera 9 / settings 9 / calibration 10 / range 8 / app 4, all passing. Live `--mock --web-port 8098`: 4 shots enriched (dots, carry range, player), Session on the Pi source with server stats, swipe-delete removed the shot on the Pi, clear via dialog, CSV share sheet, Training, Camera "Camera Not Available", Settings (debug log path, cloud "Cloud uploader is not linked."), shutdown dialog cancelled (scratchpad `r6c-android-*.png`). The mock spins carried no `spin_source`, so the spin-source subtext was only verified in device tests.
- R5b/R6c-Android ✅ merged (e0903f8). Orchestrator note: merge commit was created while a transient :androidApp:packageDebug failure was masked by a grep|head gate; re-verified clean (gradle exit 0) immediately after, no code change needed. Gating fixed going forward.
- R7a ✅ follow-ball camera (shared + Android). `core:flight.FollowCameraPlanner` (pure, analytic, smoothstep-eased; scene space, downrange −z, trajectory z mirrored): t=0 = fixed tee pose, chase from behind the ball's heading (offset grows with horizontal speed/height, target leads 0.18 s), last 20 % eases target toward landing + rises/pulls back + FOV 58→66°, then `SETTLE_SECONDS`=0.9 s into a 16 m-up view of the landing spot lined up with the nearest yardage marker (turn ≤ 50° off carry, setback 25 m or marker distance + 18 m when the marker is short). `RangeCameraPose` gained `verticalFovDegrees` (default 58); `RangeSceneDescription.markerScenePositions` (±9 m alternating). `core:data`: `RangeCameraMode {FIXED, FOLLOW}`, `SettingsRepository.rangeCameraMode`/`setRangeCameraMode` (DataStore key `rangeCameraMode`, default **FOLLOW**, default interface bodies). VM: `ToggleCameraMode`, `ReduceMotionChanged(enabled)` (route sends it), UiState `camera: RangeCameraState(mode, locked)` + `cameraMode`/`cameraModeLocked`. **Renderer API for both platforms: `RangeCameraRig().pose(mode:trajectory:playbackProgress:landedElapsedSeconds:)` → `FlightRangeCameraPose` (position/target/verticalFovDegrees), `settleSeconds`, `fixedPose`.** Android: `RangeProjection.update()`/`projectInto()` (mutable, allocation-free), world-space `RangeScene`/`WorldPolygon` + `RangeRenderer` re-projecting only when pose/size change, per-frame depth-sorted trees, tracer drawn as one filled ribbon clipped at the near plane (`TracerRibbon`), overlay "Follow"/"Fixed" toggle (`OfIcons.Camera`, new in core:designsystem). Verified: spotlessCheck detekt allTests assembleDebug linkDebugFrameworkIosSimulatorArm64 green; device tests on Pixel_10_Pro_XL feature:range:ui 12/12, androidApp 2/2 (a second AVD never came online: `adb` reported it offline after cold boot); live mock :8099 burst `r7-android-follow-*.png` + `r7-android-fixed-*.png` in scratchpad. The Float/Double comparison bug in the settle loop (infinite frame loop) was caught by the device tests and fixed before commit.
- R7a ✅ merged (882fee4). Orchestrator review of r7-android-follow-*: tracking + landing framing work. Tuning for R7b: (1) settle pose looks ~50° diagonal and the marker disc fills the foreground → settle behind the landing point looking down the flight line, marker just beyond; (2) detail-metrics panel covers lower half → compact strip during Flying/Landed on BOTH platforms. R7a worktree lock pending cleanup. Leftover AVD 'R7a_Test' created by agent.
- R5b/R6c-iOS ✅ SwiftUI UI for the parity + full-Wi-Fi VMs. Bridge (`shared/iosMain/NativeViewModels.kt`): `state`/`sideEffects` for Session/Settings/Camera, `state` for Training (it reports errors in state), and `CameraViewModel.jpegFrames: Flow<NSData>` = `frames.conflate()` + pinned copy; `SharedViewModel` conformances in `ViewModelHost.swift`. KMPNativeCoroutinesAsync's sequence holds ≤1 item and only requests the next on `next()` (checked in its source), so with `conflate()` the MJPEG view (decode `UIImage(data:).preparingForDisplay()` in a detached task, keep only the newest image, collect only while `phase == STREAMING` and scene active) never buffers unboundedly. **UI-testing repo:** `PreviewShotRepository.deleteShot/clearHistory` are no-op defaults, so iosMain `startKoinForIos()` wraps it (only under `--ui-testing`/`--preview-shot`) in `LocalEditsShotRepository`, which applies the real repo's local delete/clear to history/latestShot (shared/commonMain untouched). Nav: `TabView` Dashboard/Session/Training/Camera/Settings, each its own `NavigationStack`; Calibrate + full-screen Range stay pushed from the dashboard stack with the tab bar hidden. Dashboard: units everywhere, 3-dot `ConfidenceDots` for Launch/Spin, spin-source subtext, carry range or spin-adjusted carry, player, club-chips card, `.sensoryFeedback(.impact)` + 0.6 s radial gold `ShotFlash` (Canvas/TimelineView) on `NewShot`; degrees via shared `formatDegrees` ("9.5°"), Range overlay degrees now tight too. Session: `List` with source badge, club tabs, stats grid (swing stats when all reps), `.swipeActions` Delete (red), Clear via `confirmationDialog`, CSV via `ShareLink` of a temp `exports/<vm filename>.csv` rebuilt (ExportCsv) whenever the shot ids change, Simulate Shot when `showSimulateShot`. Training: grouped implement chips (`FlowLayout`), Last/Best/Average, error banner, disabled reasons. Camera: phase copy, ball status/confidence, toggles with reasons. Settings: `Form` with units, connection + live-session pill, player, sim pills, radar status/sliders (VM ranges, commit on release, reasons), recent triggers, debug toggle + log path, cloud status, Shut Down Pi with destructive `confirmationDialog` driven by the VM flag. Dialogs attach to their buttons: on iOS 26 a `confirmationDialog` renders as a popover **without a Cancel button** (dismiss = `PopoverDismissRegion`), which the XCUITests handle. Section-level `accessibilityIdentifier`s leak onto Form children, so none are used. Fonts: DM Serif Display + Outfit variable TTFs (+OFL texts) in `iosApp/Fonts`, `UIAppFonts`; `Font.ofDisplay/of(_:weight:)` use `.custom(_, size:, relativeTo:)` (Outfit PostScript name is `Outfit-Thin`; `.weight` drives the wght axis, verified visually), nav/tab bars via UIKit appearance with `UIFontMetrics`. New XCUITests `SessionUITests` (3) + `PiScreensUITests` (4): stats/row, swipe-delete, Clear confirm cancel+confirm, units → dashboard km/h/m (restored), Settings/Camera/Training disabled with "Not connected" (preview never starts the Pi session; shutdown confirm covered live). Verified: `spotlessCheck detekt allTests :androidApp:assembleDebug :shared:linkDebugFrameworkIosSimulatorArm64` rc=0; `xcodebuild test` on iPhone 17 Pro Max (F9696659…) rc=0: 3 unit + 17 UI (1 live skipped), 0 failures. Live `--mock --web-port 8096` (scratchpad `ref` clone; the ~/Developer/oss/openflight `main` checkout has no `/api/shots/stream`), 4 fired shots enriched (dots, carry range, player), Session on the Pi source, swipe-delete 4→3 on the Pi, Clear popover, ShareLink sheet with the CSV, Training, Camera "Not Available", Settings radar/debug/cloud, shutdown popover cancelled, metric dashboard — screenshots `r6c-ios-*.png` via a temporary XCUITest (deleted, not committed).
- R4 ✅ app icons + docs refresh. **Logo source:** the official OpenFlight favicon at
  `https://openflight.dev/icon.svg` (fetched 2026-09-24; also present as
  `ui/public/openflightlogo.svg`/`openflightdarklogo.png` in the reference repo) — a 32×32
  vector of a gold golf ball with dimples on a `#0A0A0F` tile with a gold `#D4AF37` flight-
  trail arc, already palette-matched and designed for small sizes; the traced wordmark SVG and
  `cad/logo/MainLogo.DXF` were not needed. Recentered/rescaled (`translate(8.60,12.66)
  scale(2.7106)` in a 108-unit space) so the mark sits inside Android's adaptive-icon safe zone
  under any launcher mask, with the gradient ball flattened to solid `#D4AF37` per the plan's
  fallback guidance. **Android:** `ic_launcher_background.xml` (solid `#0A0A0F`),
  `ic_launcher_foreground.xml` (vector mark), new `ic_launcher_monochrome.xml` (themed-icon
  alpha silhouette, wired into both `mipmap-anydpi-v26/ic_launcher*.xml`), legacy per-density
  `ic_launcher.png`/`ic_launcher_round.png` regenerated at mdpi–xxxhdpi via `rsvg-convert`
  (installed with `brew install librsvg`, not present before). **iOS:** `AppIcon.appiconset`
  gained `app-icon-1024.png` (replaced), `app-icon-1024-dark.png` (same art — already dark) and
  `app-icon-1024-tinted.png` (grayscale), all 1024×1024 no-alpha, `Contents.json` filenames
  wired for the light/dark/tinted appearance slots. **Docs:** README rewritten (features incl.
  Wi-Fi-only Socket.IO features and the follow camera, full module map matching
  `settings.gradle.kts`, iOS signing notes, mock-server workflow, `xcrun simctl list devices
  available` + UUID test commands, ported/adapted troubleshooting from the reference
  `ios/README.md`, known limitations, credits incl. `jake-fishtech/openflight@feat/iOS-ble`
  commit `b053194`, AGPL license); `CLAUDE.md` module graph brought current (`core:socketio`,
  `core:testing`, `core:insights`, the four new `feature:*`/`feature:*:ui` pairs) plus a new
  "Gate commits on exit code" section; `docs/hardware-test-matrix.md` marked **PENDING** (no Pi
  yet, every row BLOCKED-ON-HARDWARE) and given the reference "Verify the app" checklist.
  `.github/workflows/ci.yml` checked against current task/module names (`:shared:…`, the
  `iosApp` scheme) — already correct, no changes needed. **No tag created** (orchestrator's
  job). Verified: `spotlessCheck detekt allTests :androidApp:assembleDebug
  :shared:linkDebugFrameworkIosSimulatorArm64` rc=0; `xcodebuild build` on the iPhone 17 Pro Max
  sim (F9696659…) rc=0, asset catalog compiled `AppIcon` cleanly. Installed on
  `Pixel_10_Pro_XL` (app drawer + dock, circle-masked) and the Pro Max sim (home screen,
  squircle-masked): both crisp, centered, legible at launcher size — screenshots
  `r4-android-dock-row.png`, `r4-android-launcher.png`, `r4-ios-home-raw.png`,
  `r4-ios-icon-zoom.png` in the scratchpad.
- R7b ✅ iOS follow camera + range tuning (46c4c7a). **Settle pose** (`FollowCameraPlanner.settledPose`): behind the landing spot along the ball's *landing heading* (final velocity), turned ≤ 14° toward the next marker beyond, 13 m up / 26 m back, target = landing, FOV 64°. A short marker whose disc would reach the foreground is put behind the camera (setback pulled in to ≥ 10 m, height = 0.55 × setback, ≥ 8 m) or, if it's too close to the landing spot for that, kept ≥ 32 m in front of the camera (push-back). The push-back came from the first live burst (211-yd carry ~10 m past the 200-yd disc still filled the foreground), so a regression test covers landings 4–25 m past every marker. Parameters were swept numerically first (scratchpad `r7b/sweep*.py`). Deviation: in **portrait**, the "next marker beyond in frame" test only covers markers ≤ 12 m to the side of the flight line, because a ~30°-wide view can't reach a marker ~19 m off a big hook or slice within the ≤ 15° yaw limit; landscape covers every marker. **Compact overlay:** shared `DrivingRangeUiState.compactMetrics` (Flying/Landed) + `compactMetricsSummary` ("Club 103.2 mph · Launch 12.6° · Spin 2,380 rpm") and tags `range.metricsDetail`/`range.metricsCompact`. Android and iOS swap the detail grid (club selector + 7 metrics) for a single pill; the top tiles and controls stay. **iOS:** `RangeSceneController` poses the `PerspectiveCamera` from `RangeCameraRig` on every CADisplayLink frame through landing + `settleSeconds` (FOV onto its component), holds the pose between flights, and re-poses immediately on a mode change. `suspend()` still lets a landed settle finish. The controls get a Follow/Fixed button (`video` symbol, disabled when locked). `ReduceMotionChanged` is sent on appear and on change. iOS degree spacing was already tight ("13.6°", seen live). Verified: `spotlessCheck detekt allTests :androidApp:assembleDebug :shared:linkDebugFrameworkIosSimulatorArm64` rc=0; Pixel_10_Pro_XL `feature:range:ui` 14/14, `androidApp` 4/4 (rc=0); `xcodebuild test` on the Pro Max (F9696659…) rc=0, 3 unit + 18 UI (1 live skipped) incl. the new toggle-persistence test and compact/expand assertions in the preview-flight test. Live mock :8099: `r7b-android-*.png` and `r7b-ios-*.png` in scratchpad. Both show chase tracking, the compact strip in flight and dwell, a settled view down the flight line with the next marker beyond and no giant foreground disc, and the panel expanded afterwards.
- R7b ✅ merged (e9b779e; merged with --no-commit, gated on gradle=0 + xcode=0). v0.1.0-sim tagged. Installed on user's iPhone 17 Pro sim + Pixel emulator against demo mock server :8094; both live and verified by screenshot. Note: Android launch extra is case-sensitive ('--es transport wifi'), iOS accepts 'WIFI'.
- Published 2026-09-25: public repo https://github.com/btripp/openflight-mobile-kmp (main + v0.1.0-sim). History rewritten to GitHub no-reply email before publishing (backup bundle in session scratchpad). First CI run green: Linux lint/tests/assemble + macOS iOS tests/Xcode build.
