 ...
 *
 * The app graph is the real one ([appModules] + [applyLaunchOptions]), with the settings kept in
 * memory so no DataStore file outlives a test. They start on Wi-Fi: the test APK declares the
 * Bluetooth permissions (from core:ble), so Bluetooth would put a permission prompt over the app,
 * while the Wi-Fi LAN permission is undeclared here and is denied without one.
 */
...
class DrivingRangeFlowTest {
    ...
    private fun launch(options: LaunchOptions) {
        val koin =
            startKoin {
                modules(appModules + module { single<SettingsRepository> { InMemorySettingsRepository() } })
            }.koin
